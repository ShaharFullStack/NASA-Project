...
import { NasaFeature } from "./types/nasaTypes.js";

const displayEvents = async (events: NasaFeature[]) => {
    ...
    if (eventsContainer) {
        ...
        events.forEach(event => {
            ...
            eventElement.className = "event-card"; // Optional: Add a class for better styling control
...
document.getElementById("byDateBtn")?.addEventListener("click", async () => {
    const startDate = (document.getElementById("sinceBox") as HTMLInputElement).value;
    const endDate = (document.getElementById("untilBox") as HTMLInputElement).value;
